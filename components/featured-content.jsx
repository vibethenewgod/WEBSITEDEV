  ...
  return (
    <section className="py-16 bg-gray-50">
      <div className="container mx-auto">
        <h2 className="text-2xl md:text-3xl font-medium text-[#1A1A1A] mb-12 text-center uppercase">
          OUR PRACTICE AREAS
        </h2>
        ...
        <div className="hidden md:grid md:grid-cols-3 gap-8">
          {featuredItems.map((item, index) => (
            <div key={index} className="bg-white p-8 shadow-sm border-t-2 border-[#D8E6F3]">
              <h3 className="text-xl font-normal text-[#1A1A1A] mb-4">{item.title}</h3>
              <p className="text-gray-600 mb-6 font-normal">{item.description}</p>
              <Link
                href={item.link}
                className="inline-flex items-center text-[#1E72C8] font-normal hover:text-[#6BBFEA]"
              >
                {item.linkText}
        ...
        <div className="md:hidden space-y-6">
          {featuredItems.map((item, index) => (
            <div key={index} className="bg-white shadow-sm flex">
              ...
              <div className="w-2/3 p-4">
                <div className="text-[#1A1A1A] text-xs font-normal mb-1">PRACTICE AREA</div>
                <h3 className="text-sm font-normal text-[#1A1A1A] mb-2">{item.title}</h3>
                <Link href={item.link} className="text-sm text-[#1E72C8] font-normal hover:text-[#6BBFEA]">
                  Read More
                </Link>
              </div>
            </div>
          ...
          <div className="text-center mt-6">
            <Link
              href="#all-services"
              className="inline-block px-6 py-2 border border-[#1E72C8] text-[#1E72C8] font-normal text-sm hover:bg-[#1E72C8] hover:text-white transition-colors"
            >
              View All Services
            </Link>
          </div>
        ...
        <div className="mt-10 text-center hidden md:block">
          <Link
            href="#all-services"
            className="inline-flex items-center justify-center px-6 py-3 bg-[#1E72C8] text-white font-normal hover:bg-[#6BBFEA] transition-colors"
          >
            View All Services
          </Link>
        </div>
